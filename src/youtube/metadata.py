...
class MetadataGenerator:
    ...
    def _generate_description(
        ...
        if template:
            ...
            # Default description
            pdf_part = f"\n{pdf_section}\n" if pdf_section else "\n"
            description = f"""History Class for {date} | History Academy

📌 Topics Covered:
{topics_list}
{pdf_part}
🔔 Subscribe for daily history classes!
📚 Like & Share to help fellow aspirants!

#History #IndianHistory #UPSC #HistoryClass #StatePSC #UPSCHistory
"""
    ...
    def _generate_tags(
        ...
        today = datetime.now()
        tags.extend([
            f"history class {today.strftime('%B %Y')}",
            f"UPSC history {today.strftime('%Y')}",
            f"Indian history {today.strftime('%Y')}"
        ])
